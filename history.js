...
async function init(){
  ...
  try{current=await getJson('./data/sgn-day.json');}
  catch{
    try{current=await getJson('https://raw.githubusercontent.com/kenzuko/vietnam-airport-live/main/data/sgn.json');current={...current,live_only:true,complete_day:false};}catch{}
  }
  try{index=await getJson('./data/history-index.json');}catch{}
  const options=[];
  if(current?.date)options.push({date:current.date,url:current.live_only?'https://raw.githubusercontent.com/kenzuko/vietnam-airport-live/main/data/sgn.json':'./data/sgn-day.json',current:true,live_only:current.live_only});
  for(const d of index.days||[]){if(!options.some(x=>x.date===d.date))options.push({date:d.date,url:`./data/${String(d.file||'').replace(/^\.\//,'')}`});}
...
$('dateSelect').addEventListener('change',e=>loadDay(e.target.value).catch(err=>$('historyHealth').textContent=`Lỗi: ${err.message}`));$('historySearch').addEventListener('input',filterRows);init().catch(err=>$('historyHealth').textContent=`Không đọc được lịch sử: ${err.message}`);
